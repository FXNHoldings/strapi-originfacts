...
const cards = docs.map((doc) => {
  const rows = doc.proposals.map((p) => `<tr class="${esc(p.status)}"><td>${esc(p.module)}.${esc(p.field)}</td><td>${esc(p.value)}</td><td>${esc(p.confidence)} (${esc(p.confidence_band)})</td><td>${esc(p.status)}</td><td>${esc(p.flags.join(', ') || '—')}</td><td>${esc(p.evidence)}</td><td><a href="${esc(p.source_url)}" rel="noreferrer">source</a></td></tr>`).join('');
  return `<section><h2>${esc(doc.carrier_key)}</h2><p>${doc.summary.field_proposals ?? 0} proposals · ${doc.summary.high_confidence ?? 0} high confidence · ${doc.summary.needs_review ?? 0} need review · ${doc.summary.unclassified_candidates ?? 0} unclassified</p>${rows ? `<div class="table"><table><thead><tr><th>Field</th><th>Value</th><th>Confidence</th><th>Status</th><th>Flags</th><th>Evidence</th><th>Official page</th></tr></thead><tbody>${rows}</tbody></table></div>` : '<p>No field proposals from the available evidence.</p>'}</section>`;
}).join('\n');
const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Airline fact review</title><style>body{font:15px/1.45 system-ui;margin:0;background:#f6f8fb;color:#172033}main{max-width:1500px;margin:auto;padding:32px}h1{margin:0 0 8px}section{background:white;border:1px solid #dce3ec;border-radius:8px;padding:20px;margin:20px 0}.table{overflow:auto}table{border-collapse:collapse;width:100%;min-width:1000px}th,td{text-align:left;vertical-align:top;border-bottom:1px solid #e7ecf2;padding:9px}th{background:#f5f7fa;position:sticky;top:0}.needs_review td{background:#fffaf0}a{color:#0759b8}</style></head><body><main><h1>Airline fact proposal review</h1><p>Generated ${esc(manifest.generated_at)}. Everything is <strong>unreviewed</strong>; this report cannot publish facts.</p>${cards}</main></body></html>`;
